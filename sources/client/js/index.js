...
document.addEventListener('DOMContentLoaded', () => {
	...
	for (let i = 0; i < dayNumber.length; i++) {
		...
		let weekend = dayNumber[i].parentNode;
		weekend.dataset.timeStamp = timestamp;
		if ((dayWeek[i].innerHTML == 'Вс') || (dayWeek[i].innerHTML == 'Сб')) {
			weekend.classList.add('page-nav__day_weekend');
		} else {
			weekend.classList.remove('page-nav__day_weekend');
		}
	}
